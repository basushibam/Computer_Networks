		...
		if(sret==0){
			...
			if(FD_ISSET(t1fd,&readfds)){
				...
				if(clients.find(clientName)!=clients.end()){
					if(service=="service1"){
						pthread_t s1; string temp = service;
						pthread_create(&s1,NULL,&service1,(void*)&temp);
						
					}
					if(service=="service2"){
						pthread_t s2; string temp = service;
						pthread_create(&s2,NULL,&service2,(void*)&temp);
						
					}
					if(service=="service3"){
						pthread_t s3; string temp = service;
						pthread_create(&s3,NULL,&service3,(void*)&temp);
